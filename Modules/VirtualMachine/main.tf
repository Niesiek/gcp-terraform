data "google_compute_network" "subnet" {
...
resource "google_compute_instance" "main" {
  ...
  network_interface {
    subnetwork = data.google_compute_network.subnet.self_link
  ...
  metadata = {
    ...
    ssh-keys = join("\n", [
    "user:${file("${path.module}/ssh/ssh-key.pub")}",
    "user2:${file("${path.module/ssh/ssh-key2.pub}")}"
    ])
  }
}
